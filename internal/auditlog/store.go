...
func (s *Store) LogAPIRequest(ctx context.Context, method, path string, statusCode int, userName string, durationMs int, clientIP string) error {
	...
	_, err := s.pool.Exec(ctx, `
		INSERT INTO qc_api_audit (method, path, status_code, user_name, duration_ms, client_ip)
		VALUES ($1, $2, $3, $4, $5, $6)`,
...
func (s *Store) ListAPIAuditLogs(ctx context.Context, limit int) ([]map[string]any, int, error) {
	...
	var total int
	if err := s.pool.QueryRow(ctx, `SELECT COUNT(*)::int FROM qc_api_audit`).Scan(&total); err != nil {
		return nil, 0, err
	}
	rows, err := s.pool.Query(ctx, `
		SELECT method, path, status_code, user_name, duration_ms, logged_at
		FROM qc_api_audit ORDER BY logged_at DESC LIMIT $1`, limit)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
